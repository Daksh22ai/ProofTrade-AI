...
import threading
import signal
import uuid            # used by PingPongGuard for unique ping req_id
import asyncio
...
class SystemConfig:
    ...
    ws_processor_threads: int = max(3, os.cpu_count() // 2)
    ...
    # Queue Configuration
    ws_raw_queue_size: int = 200000  # Memory buffer size
    db_write_queue_size: int = 300000
    
    # Raw Message Logging (THE KEY COMPONENT)
    raw_log_dir: str = "raw_ws_logs"
    raw_log_buffer_size: int = 1024 * 1024  # 1MB buffer for fast writes
    raw_log_file_size_mb: int = 100
    raw_log_retention_days: int = 1  # Auto-delete raw logs after 1 days
...
def _questdb_max_ts(table: str, symbol: str, interval: str = None, host: str = None) -> Optional[int]:
    ...
    return int(val) // 1000 if int(val) > 2_000_000_000_000 else int(val)
...
def ensure_kafka_partitions(bootstrap: str, topic: str = 'bybit-market-data'):
    """
...
_DEDUP_DDL = [
    ...
    "ALTER TABLE liquidations    DEDUP ENABLE UPSERT KEYS(timestamp, symbol)",
]
...
# === OpenTelemetry Setup ===
# No exporter attached — spans are discarded. To enable distributed tracing,
# attach an OTLP gRPC BatchSpanProcessor before calling set_tracer_provider().
...
class OptimizedDatabaseManager:
    ...
    def _format_ilp_line_from_topic(self, internal_topic: str, data_item: dict) -> Optional[str]:
        ...
        try:
            ...
            elif "liquidation" in internal_topic:
...
class StreamSchemas:
    """Pydantic models retained for REST validation (lower throughput, field-level errors needed)."""
...
class RestDataFetcher:
    ...
    async def stop(self):
        ...
        # enqueue one None per worker to unblock them
        for _ in self.workers:
            asyncio.run_coroutine_threadsafe(self.queue.put(None), loop)
        # await all worker tasks to finish
    ...
    def _fetch_klines(self, symbol: str, interval: str, producer: KafkaProducer, start_ts: int, end_ts: int):
        ...
        # Bybit retCodes that are transient — back off and retry
        RETRYABLE_CODES = {
            10004,   # request timeout
            10016,   # internal server error
            10400,   # rate limit exceeded (alternative)
            110001,  # order rate limit
            3400008, # too many requests
        }
        # Bybit retCodes that signal a permanent problem — abort
        FATAL_CODES = {
            10001,   # invalid symbol
            10003,   # invalid api_key (shouldn't matter for public endpoints)
            10017,   # feature not available
        }
        MAX_RETRIES = 5
        ...
        while start_ts < end_ts:
            ...
            for attempt in range(MAX_RETRIES):
                try:
                    ...
                    # Transient: TCP failure, timeout, FailedRequestError (pybit exhausted its
                    # 3 internal retries on 10006/10002), or ConnectionError.
                    # pybit already handled 10006 by reading reset header and sleeping.
            ...
            if page_batch:
                ...
                producer.send('bybit-market-data', key=symbol.encode(), value=message)
    ...
    def _fetch_open_interest(self, symbol: str, interval: str, producer: KafkaProducer, start_ts: int, end_ts: int):
        ...
        RETRYABLE_CODES = {10004, 10016, 10400, 3400008}
        FATAL_CODES     = {10001, 10003, 10017}
        MAX_RETRIES     = 5
        BASE_BACKOFF    = 1.0
...
class Librarian:
    ...
    def _find_candle_gaps(self) -> list:
        ...
        def _to_ms(val) -> int:
            """Convert QuestDB TIMESTAMP to epoch ms — handles both ISO 8601 and int."""
            if val is None:
                return 0
            if isinstance(val, str):
                try:
                    ...
                    return 0
            v = int(val)
            return v // 1000 if v > 2_000_000_000_000 else v  # µs → ms if needed

        gaps = []
        for row in result.get("dataset", []):
            try:
                symbol, interval, gap_start_raw, gap_end_raw = row
                gap_start_ms = _to_ms(gap_start_raw)
                gap_end_ms   = _to_ms(gap_end_raw)
...
class BybitWsFeed:
    ...
    def subscribe(self, symbols: list):
        """Subscribe to all configured streams for the given symbols."""
        ...
        for symbol in symbols:
            try:
                # Kline streams (one per configured interval)
                ...
                # Liquidations (use all_liquidation_stream — deprecated liquidation_stream warns)
                ws.all_liquidation_stream(symbol=symbol, callback=cb)
            except Exception as e:
                logger.error(f"[WsFeed {id(self)}] Subscribe failed for {symbol}: {e}")
...
class BybitWsManager(threading.Thread):
    ...
    def __init__(self, producer, stop_event):
        """
        Parameters
        ----------
    ...
    def _health_sweep(self):
        ...
        with self._rlock:
            for feed in self._feeds:
                # Skip feeds still in warmup — they're connecting
                warmup = getattr(feed, "_connected_at", None)
                if warmup is None:
                    continue   # not yet connected; let the connect thread finish
        ...
        for feed in dead:
            syms = feed.symbols
            dead_count = metrics.ws_disconnects + 1
            metrics.ws_disconnects = dead_count
            feed.disconnect_ts = int(time.time() * 1000)
            logger.warning(
                f"[WsManager] ⚠️  Feed {id(feed)} is DEAD (disconnect #{dead_count}). "
    ...
    while not stop_event.is_set():
        ...
        for msg in batch:
            try:
                ...
                producer.send('bybit-market-data', key=_ws_key, value=msg)
                metrics.kafka_sent += 1
            except Exception as e:
...
async def main():
    ...
    ws_manager = WebSocketManager(producer, stop_event)
    ...
    # Start rest_fetcher worker pool — do not assume it returns an awaitable task.
    # If rest_fetcher.start() spawns background workers synchronously, call it directly.
    start_result = rest_fetcher.start()
    if asyncio.iscoroutine(start_result):
        await start_result


    # === STAGE 4: RUN HISTORICAL FETCH ===
    logger.info("Starting initial historical data fetch...")
    await rest_fetcher.enqueue_historical(symbols)
    await rest_fetcher.queue.join()
    await asyncio.sleep(0.5)
    logger.info("✅ Historical data fetch and processing complete.")

    # Apply DEDUP UPSERT KEYS to all tables now that they exist from the first ILP writes.
    # Idempotent: safe on every restart; QuestDB skips if already enabled.
    await asyncio.to_thread(ensure_table_dedup)

    # === STAGE 5: START REAL-TIME AND SCHEDULED TASKS ===
    ...
    # Populate the symbol queue BEFORE starting the thread so the first
    # _drain_queue() call in run() already has symbols to process.
    ws_manager.add_symbols(symbols)

    # WebSocketManager is a threading.Thread — start it as a thread, not an asyncio task
    ws_manager.start()
    _ws_manager = ws_manager   # wire up health endpoint

    # Drain coroutine: pulls validated WS messages from _ws_raw_queue and sends to Kafka
    drain_task = asyncio.create_task(drain_to_kafka(producer, stop_event), name="WS-Drain")

    # REST scheduler daemon thread — uses run_coroutine_threadsafe to enqueue safely
    rest_scheduler_thread = start_rest_scheduler(rest_fetcher, loop)

    # Phase 6: Librarian — gap scanner & auto-repair
    # Starts after backfill completes; 90s internal cooldown before first scan.
    ...
    try:
        ...
        # Close db_manager resources (ILP socket)
        if hasattr(db_manager, "close"):
            ...
            # best-effort: try to shutdown any writer API
            if hasattr(db_manager, "shutdown"):
                try:
                    await asyncio.to_thread(db_manager.shutdown)
                except Exception:
                    pass
